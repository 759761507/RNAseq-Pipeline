...
def parse_gtf_exons(gtf_path):
    ...
    site_anno = defaultdict(lambda: {"gene_ids": set(), "gene_names": set(), "transcript_ids": set()})
    ...
    # Merge exon intervals for scanning while retaining per-site annotation later.
...
def count_site(bams, chrom, pos0, ref_base, edited_base, min_baseq):
        ...
        for (chrom, strand), intervals in merged_exons.items():
            for start, end, payload in intervals:
                ...
                for i, base in enumerate(seq):
                    ...
                    counts, edited, total, rate = count_site(bams, chrom, pos0, genomic_ref_base, edited_base, args.min_baseq)
